...
public class UITest : UIFigure
{
	[SerializeField]
	private List<Vector2> _Points = new List<Vector2>();

	[SerializeField]
	private List<Vector2> _Points1 = new List<Vector2>();
	...
	protected override void OnPopulateMesh(VertexHelper p_vertexHelper)
	{
		...
		list = new List<UIVertex>(_Points1.Count);
		if (_Points1.Count >= 3)
		{
			for (int j = 0; j < _Points1.Count; j++)
			{
				list.Add(AddVertex(_Points1[j]));
			}
			p_vertexHelper.AddUIVertexStream(list, (!_UseTriangleFan) ? FigureTopology.TringleStripIndices(list.Count - 2) : FigureTopology.TringleFanIndices(list.Count - 2));
		}
	}
