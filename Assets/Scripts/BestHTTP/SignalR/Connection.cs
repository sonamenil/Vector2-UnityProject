...
namespace BestHTTP.SignalR
{
	public sealed class Connection : IHeartbeat, IConnection
	{
		...
		void IHeartbeat.OnHeartbeatUpdate(TimeSpan dif)
		{
			...
			DateTime? reconnectStartedAt = ReconnectStartedAt;
			if (reconnectStartedAt.HasValue)
			{
				DateTime? reconnectStartedAt2 = ReconnectStartedAt;
				TimeSpan? timeSpan2 = ((!reconnectStartedAt2.HasValue) ? null : new TimeSpan?(DateTime.UtcNow - reconnectStartedAt2.Value));
				if (timeSpan2.HasValue && timeSpan2.Value >= NegotiationResult.DisconnectTimeout)
				{
		...
		public void Send(object arg)
		{
			...
			lock (SyncRoot)
			{
				if (State == ConnectionStates.Connected)
				{
					string jsonStr = JsonEncoder.Encode(arg);
