...
namespace BestHTTP.SignalR.Hubs
{
	public class Hub : IHub
	{
		...
		void IHub.OnMethod(MethodCallMessage msg)
		{
			...
			HTTPManager.Logger.Information("Hub - " + Name, string.Format("[Client] {0}.{1} (args: {2})", Name, msg.Method, msg.Arguments.Length));
		}
		...
		public void Call(string method, OnMethodResultDelegate onResult, OnMethodFailedDelegate onResultError, OnMethodProgressDelegate onProgress, params object[] args)
		{
			lock (((IHub)this).Connection.SyncRoot)
			{
				...
				((IHub)this).Call(new ClientMessage(this, method, args, ((IHub)this).Connection.ClientMessageCounter++, onResult, onResultError, onProgress));
			}
		}
