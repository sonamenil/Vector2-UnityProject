...
namespace BestHTTP.SignalR.Transports
{
	public abstract class TransportBase
	{
		...
		protected void Start()
		{
			...
			HTTPRequest hTTPRequest = new HTTPRequest(Connection.BuildUri(RequestTypes.Start, this), HTTPMethods.Get, true, true, OnStartRequestFinished);
			hTTPRequest.Tag = 0;
		...
		private void OnStartRequestFinished(HTTPRequest req, HTTPResponse resp)
		{
			...
			if (state == HTTPRequestStates.Finished)
			{
				if (resp.IsSuccess)
				{
					...
					if (text != "started")
					{
						Connection.Error(string.Format("Expected 'started' response, but '{0}' found!", text));
						return;
					}
					State = TransportStates.Started;
					Started();
				...
				HTTPManager.Logger.Warning("Transport - " + Name, string.Format("Start - request finished Successfully, but the server sent an error. Status Code: {0}-{1} Message: {2} Uri: {3}", resp.StatusCode, resp.Message, resp.DataAsText, req.CurrentUri));
			}
		...
		public virtual void Abort()
		{
			if (State == TransportStates.Started)
			{
				...
				HTTPRequest hTTPRequest = new HTTPRequest(Connection.BuildUri(RequestTypes.Abort, this), HTTPMethods.Get, true, true, OnAbortRequestFinished);
				hTTPRequest.Tag = 0;
				hTTPRequest.DisableRetry = true;
		...
		private void OnAbortRequestFinished(HTTPRequest req, HTTPResponse resp)
		{
			...
			if (state == HTTPRequestStates.Finished)
			{
				...
				HTTPManager.Logger.Warning("Transport - " + Name, string.Format("Abort - Handshake request finished Successfully, but the server sent an error. Status Code: {0}-{1} Message: {2} Uri: {3}", resp.StatusCode, resp.Message, resp.DataAsText, req.CurrentUri));
			}
