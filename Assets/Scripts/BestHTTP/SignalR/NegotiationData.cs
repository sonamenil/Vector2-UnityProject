	...
	public sealed class NegotiationData
	{
		...
		private void OnNegotiationRequestFinished(HTTPRequest req, HTTPResponse resp)
		{
			...
			case HTTPRequestStates.Finished:
				...
				{
					RaiseOnError(string.Format("Negotiation request finished Successfully, but the server sent an error. Status Code: {0}-{1} Message: {2} Uri: {3}", resp.StatusCode, resp.Message, resp.DataAsText, req.CurrentUri));
				}
				break;
